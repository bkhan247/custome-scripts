...
scriptInjection("https://code.jquery.com/jquery-3.2.1.min.js", function () {
    ...
    if (Shopify.shop == "jointhecreativeside.myshopify.com") {
        $jq321("head").append(
          '<style type="text/css"> .view-full-details a{color: #F2BCB8 !important} .quick-shop-addtocart button{background-color: #F2BCB8 !important; color: #fff !important;} .quickshop-quantity input{ border: 1px solid #F2BCB8 !important} .quantity-button.quantity-up{border-color: #F2BCB8 !important;} .quantity-button.quantity-down{border-color: #F2BCB8 !important;} </style>'
        );
    }
    ...
    function collectionQuickView(quickViewCollectionText, quickViewCollectionLayout, quickViewCollectionPosition) {
        ...
        if (product_id == '') {
            ...
            if (($jq321.inArray("products", currentPageHandle) != -1) || ($jq321.inArray("collections", currentPageHandle) != -1)) {
                $jq321("main").find('a').each(function () {
                    if (typeof ($jq321(this).attr('href')) != 'undefined') {
                        if ($jq321.inArray($jq321(this).attr('href').trim(), allLinks) == -1) {

                            var href = $jq321(this).attr('href');
                            var url = href.split("/");

                            if ($jq321.inArray("products", url) != -1) {
                                allLinks.push($jq321(this).attr('href'));
                            }
                        }
                    }
                });
